...
export default function GroupingPage() {
  const { data, error, isLoading } = useSWR<{ records?: Array<{ id: string; fileNumber: string; trackingId: string; serialNo: number; schedule: string; filePrefix: string }>; fallback?: boolean }>('/api/file-index', fetcher, { revalidateOnFocus: false })
  const [rows, setRows] = useState(initialRows)
  const databaseRows = data?.records?.map((record) => ({ file: record.fileNumber, owner: `${record.schedule} schedule`, documents: 0, trackingId: record.trackingId, status: 'Indexed' })) ?? []
  const displayRows = databaseRows.length > 0 ? databaseRows : rows
  const [query, setQuery] = useState('')
  const visible = displayRows.filter((row) => `${row.file} ${row.owner} ${row.trackingId}`.toLowerCase().includes(query.toLowerCase()))
