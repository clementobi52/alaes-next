...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { recordset } = await query(`SELECT Id AS id, Schedule AS schedule, FilePrefix AS filePrefix, SerialNo AS serialNo, FileNumber AS fileNumber, TrackingId AS trackingId, CreatedAt AS createdAt, UpdatedAt AS updatedAt FROM dbo.FileIndexRecords WHERE Id = @id`, { id })
