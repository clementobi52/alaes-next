...
function Row({
  ...
  if (href) {
    return (
      <Link href={href} onClick={() => { onClick?.(); onNavigate?.() }} className={className} aria-current={isActive ? 'page' : undefined}>
        {children}
      </Link>
    )
  }
...
function NavBranch({
  ...
  const isOpen = expanded.has(path)
  const isActive = node.href ? currentRoute === node.href : active === path
