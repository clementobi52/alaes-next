    ...
    <Modal open onClose={onClose} title="Search Report Preview" widthClass="max-w-[95vw]" printable footer={<div className="flex justify-end gap-3"><button type="button" onClick={onClose} className="rounded-lg border border-border px-4 py-2 text-sm font-medium hover:bg-muted">Close</button><button type="button" onClick={() => window.print()} className="inline-flex items-center gap-2 rounded-lg bg-primary px-5 py-2 text-sm font-semibold text-primary-foreground"><Printer className="h-4 w-4" /> Print report</button></div>}>
      <article className="legal-search-report relative mx-auto min-h-[7.6in] w-[11in] max-w-full overflow-visible bg-card px-[0.4in] py-[0.2in] text-foreground shadow-sm print:min-h-0 print:w-full print:max-w-none print:shadow-none">
        ...
        <div className="relative z-10">
          ...
          <div className="report-property-details mt-1 grid grid-cols-2 gap-x-8 gap-y-2 border-b border-border pb-3 text-[11px] md:grid-cols-4"><ReportField label="File Number" value={record.fileNo} /><ReportField label="File Number ID" value={record.propertyId ?? record.id} /><ReportField label="File Title (Current Holder)" value={holder} /><ReportField label="Schedule" value={record.scheduleName} /><ReportField label="Layout" value={record.layoutName} /><ReportField label="Plot No." value={record.plotNumber} /><ReportField label="Plot Size" value={record.plotSize} /><ReportField label="Land Use" value={record.landUse} /><ReportField label="LGA / City" value={record.lga} /><ReportField label="District" value={record.district} /><ReportField label="Registration No." value={record.registrationNumber ?? record.parentRegistration} /><ReportField label="Plan No." value={record.planNumber} /><ReportField label="Approval" value={record.approved ?? record.status} /><ReportField label="Plot Description" value={record.propertyDescription ?? record.address} /></div>
          <div className="mt-3 flex items-center gap-2"><span className="inline-block border border-foreground px-2 py-1 text-xs font-bold">File History</span><span className="text-[10px] text-muted-foreground">{loadingHistory ? 'Loading full history…' : `${rows.length} transaction${rows.length === 1 ? '' : 's'} on this property`}</span></div>
          <div className="mt-1 overflow-x-auto"><table className="report-transaction-table w-full min-w-[900px] table-fixed border-collapse text-[10px]"><thead><tr className="border-b-2 border-foreground text-left"><th className="w-[3%] py-1">S/N</th><th className="w-[11%] py-1">File No.</th><th className="w-[13%] py-1">Instrument / Transaction Type</th><th className="w-[12%] py-1">Party 1 (Grantor)</th><th className="w-[12%] py-1">Party 2 (Grantee)</th><th className="w-[12%] py-1">Party 3 (Guarantor)</th><th className="w-[9%] py-1">Transaction Date</th><th className="w-[9%] py-1">Reg. Time / Date</th><th className="w-[10%] py-1">Particulars</th><th className="w-[5%] py-1">Caveat</th><th className="w-[4%] py-1">Comments</th></tr></thead><tbody>{rows.map((transaction, index) => <tr key={`${transaction.id ?? transaction.fileNo ?? record.fileNo}-${index}`} className="border-b border-border align-top"><td className="py-2">{index + 1}</td><td className="py-2 font-mono">{transaction.fileNo || record.fileNo || '—'}</td><td className="py-2">{transaction.transactionType || 'Property transaction'}</td><td className="py-2">{transaction.grantor || grantor || 'Abia State Government'}</td><td className="py-2">{transaction.grantee || record.owner || 'Not recorded'}</td><td className="py-2">{transaction.guarantor || '—'}</td><td className="py-2">{formatDate(transaction.instrumentDate || transaction.created)}</td><td className="py-2">{formatDate(transaction.registrationDate)}</td><td className="py-2">{transaction.registrationNumber || transaction.parentRegistration || 'Not recorded'}</td><td className="py-2">{transaction.caveat || 'Nil'}</td><td className="py-2">{transaction.propertyDescription || '—'}</td></tr>)}</tbody></table><p className="mt-3 text-center text-[10px] font-bold tracking-wide">{loadingHistory ? '' : '*** END OF TRANSACTION HISTORY ***'}</p></div>
